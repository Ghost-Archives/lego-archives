...
zen3d.OrbitControls = function(object, domElement) {
	...
	var dollyDelta = new zen3d.Vector2();
        ...
        // for setting new offset
        var raycaster = new zen3d.Raycaster();
        var raycastResult = [];
        var cameraSpacePos = new zen3d.Vector3();
	...
	function onMouseDown(event) {
		...
		event.preventDefault();
                ...
                if ( state === STATE.NONE && event.button === scope.mouseButtons.ORBIT )
                {
                    cameraSpacePos.x = ( event.clientX / App.stageW ) * 2 - 1;
                    cameraSpacePos.y = - ( event.clientY / App.stageH ) * 2 + 1;
                    cameraSpacePos.z = 1;
                
                    raycaster.setFromCamera( cameraSpacePos, scope.object );
                    raycastResult = raycaster.intersectObjects( App.scene.children );
                    
                    if ( raycastResult.length > 0 )
                    {
                        scope.target.copy( raycastResult[0].point );
                    }
                }
                */
